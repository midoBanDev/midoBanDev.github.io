...
>`COPY`  
COPY 지시어는 실행 할 때마다 임시(빌드) 컨테이너가 생성된다. 이 빌드 컨테이너는 FROM 명령에 지정된 이미지를 토대로 만들어진 컨테이너 환경을 제공한다.
COPY 지시어는 `빌드 컨텍스트에 있는 파일`을 복사하여 실행된 빌드 컨테이너 내부의 `파일 시스템의 경로`에 붙여 넣는 동작을 수행하게 된다.  
(동일한 파일이 있는 경우 덮어쓰기)  
...
VM과의 비교를 통해 알아보자. VM은 레이어 구조가 아니기 때문에 새로운 VM을 만들기 위해서는 OS와 소프트웨어를 모두 설치해야 한다.  
