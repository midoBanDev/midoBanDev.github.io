...
## JPA 동작
JPA는 Persistence라는 클래스가 있습니다. Persistence 클래스는 Persistence.xml 설정 정보를 읽어서 EntitiManagerFactory라는 클래스를 만듭니다. 
그리고 필요할 때마다 EntitiManagerFactory에서 EntityManager를 생성해서 JPA를 작동시킨다. 

```java
public static void main(String[] args) {
		/**
		 * EntityManagerFactory는 애플리케이션 로딩 시점에 한 번만 생성하여 애플리케이션 전체에서 공유한다. 데이터베이스당 1개만 실행한다.
		 * persistence.xml의 persistence-unit name 과 맵핑된다.
		 */
		EntityManagerFactory emf = Persistence.createEntityManagerFactory("hello");
		
		// EntityManager는 트랜젝션이 일어나는 시점에 계속 만들어 준다.
    // 엔티티 매니저는 쓰레드간에 공유되지 않으며, 사용한 후 버려야 한다.
    ...
    try {
			   ...
			   * 트랜잭션 안에서는 데이터 변경된 경우 JPA가 엔티티 변경 여부를 체크하여 변경이 된 경우 트랜잭션 commit 시 업데이트 쿼리가 실행된다.
  			*/
  			Member member = em.find(Member.class, 10L);
	  		member.setName("HIHI");
			  ...
			  // 5. JPQL
        // 엔티티 객체를 대상으로 하는 'SQL을 추상화한 객체지향 SQL 언어'
